...
display.set_icon(EnemiesImage0)
backgroundRect = pygame.Rect(0, 0, 400, 800)
buttonPlay = pygame.Rect(250-(cfg.BUTTON_WIDTH//2),
                         height//1.95, cfg.BUTTON_WIDTH, cfg.BUTTON_HEIGHT)
buttonOptions = pygame.Rect(
    width/2-(cfg.BUTTON_WIDTH//2), height//1.95, cfg.BUTTON_WIDTH, cfg.BUTTON_HEIGHT)
buttonExit = pygame.Rect((width-250)-(cfg.BUTTON_WIDTH//2),
                         height//1.95, cfg.BUTTON_WIDTH, cfg.BUTTON_HEIGHT)
...
while True:
    ...
    backgroundStartImage = pygame.transform.scale(
        backgroundImage, (width, height))
    screen.blit(backgroundStartImage, backgroundRect)
    mainBlock = createRectWithImage(
        pos_x, pos_y, cfg.MAINANCHO, cfg.MAINALTO, color=cfg.WHITE, image=mainBlockImg)
    createText(startFont, f'Press Start to start playing.',
               True, cfg.BLACK, screen, (width/2, 200))
    createText(startFont, f'Max Score: {maxScoreFileData}',
               True, cfg.BLACK, screen, (100, (height-100)))
    createText(startFont, f'Attempts: {attemptsFileData}',
               True, cfg.BLACK, screen, (width - 100, (height-100)))
    pygame.display.flip()
    liveCounter = 3
    scoreCounter = 0
    is_running = True
    mute = waitUserClick(buttonPlay, buttonOptions,
                         buttonExit, screen, menuState)
    # waitUser()
...
# DETECTO EVENTOS
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1:
                    # Creo un shot
                    shotsList.append(createShot(
                        mainBlock['rect'].x, mainBlock['rect'].y, bulletImg, offsetBlock=cfg.MAINANCHO//2))
            ...
            if event.type == powerUpFallingEvent:
                ...
                powerUpList.append(createRectWithImage(
                    left=width+ancho, top=randint(50, 450), ancho=60, alto=45, image=powerUpImage, vidas=1))
            if event.type == fallingBlock:
                i = randint(0, 1)
                if i == 0:
                    blockList.append(createRectWithImage(left=width+ancho, ancho=ancho,
                                     alto=alto, top=randint(100, 400), image=enemiesImages[i], vidas=1))
                else:
                    blockList.append(createRectWithImage(
                        left=width+ancho, ancho=ancho, alto=alto, top=randint(50, 450), image=enemiesImages[i], vidas=3))
...
# Blit De Textos, background y main block
        ...
        createText(my_font, f'Score: {scoreCounter}',
                   True, cfg.BLACK, screen, (200, 50))
        createText(my_font, f'Lives: {liveCounter}',
                   True, cfg.BLACK, screen, ((width-200), 50))
            ...
            if detectCollisionRect(powerUp, mainBlock):
                if poweredUp == False:
                    ...
                    pygame.time.set_timer(
                        restartPowerUp, cfg.RESTART_POWERUP, 1)
                powerUpList = borrarItemLista(powerUpList, powerUp)
...
# AFUERA DEL IS_RUNNING
    ...
    screen.blit(backgroundImage, backgroundRect)
    createText(startFont, f'Game Over !', True, cfg.BLACK,
               screen, (width/2, (height - 100)/2))
    # screen.blit(gameOver,((width -gameOver.get_width()) /2,(height - 100)/2))
    createText(startFont, f'Press any key to continue', True,
               cfg.BLACK, screen, (width/2, (height - 50)/1.7))
